...
multiQueryStmt:
    queryStmt (SEMICOLON queryStmt)* SEMICOLON?
    ;
...
createStmt
    ...
    | (ATTACH | CREATE) MATERIALIZED VIEW (IF NOT EXISTS)? tableIdentifier uuidClause? clusterClause? tableSchemaClause? (destinationClause | engineClauCse POPULATE?) subqueryClause  # CreateMaterializedViewStmt
...
// locals [std::set<std::string> attrs]:
    ...
    ( // {!$attrs.count("default")}?
      DEFAULT literal
      // {$attrs.insert("default");}
    | // {!$attrs.count("expression")}?
      EXPRESSION columnExpr
      // {$attrs.insert("expression");}
    | // {!$attrs.count("hierarchical")}?
      HIERARCHICAL
      // {$attrs.insert("hierarchical");}
    | // {!$attrs.count("injective")}?
      INJECTIVE
      // {$attrs.insert("injective");}
    | // {!$attrs.count("is_object_id")}?
      IS_OBJECT_ID
      // {$attrs.insert("is_object_id");}
    )*
    ;
dictionaryEngineClause:
// locals [std::set<std::string> clauses]:
    dictionaryPrimaryKeyClause?
    ( // {!$clauses.count("source")}?
      sourceClause
      // {$clauses.insert("source");}
    | // {!$clauses.count("lifetime")}?
      lifetimeClause
      // {$clauses.insert("lifetime");}
    | // {!$clauses.count("layout")}?
      layoutClause
      // {$clauses.insert("layout");}
    | // {!$clauses.count("range")}?
      rangeClause
      // {$clauses.insert("range");}
    | // {!$clauses.count("settings")}?
      dictionarySettingsClause
      // {$clauses.insert("settings");}
    )*
    ;
...
// locals [std::set<std::string> clauses]:
    ...
    ( // {!$clauses.count("orderByClause")}?
      orderByClause
      // {$clauses.insert("orderByClause");}
    | // {!$clauses.count("partitionByClause")}?
      partitionByClause
      // {$clauses.insert("partitionByClause");}
    | // {!$clauses.count("primaryKeyClause")}?
      primaryKeyClause
      // {$clauses.insert("primaryKeyClause");}
    | // {!$clauses.count("sampleByClause")}?
      sampleByClause
      // {$clauses.insert("sampleByClause");}
    | // {!$clauses.count("ttlClause")}?
      ttlClause
      // {$clauses.insert("ttlClause");}
    | // {!$clauses.count("settingsClause")}?
      settingsClause
      // {$clauses.insert("settingsClause");}
    )*
    ;
...
groupByClause: GROUP BY ((CUBE | ROLLUP) LPAREN columnExprList RPAREN | columnExprList) groupByRollupClause? groupByTotalsClause?;
groupByRollupClause: WITH (CUBE | ROLLUP);
groupByTotalsClause: WITH TOTALS;
...
withExpr: (identifier AS LPAREN selectUnionStmt RPAREN) | (columnExpr AS identifier);
...
orderExpr: columnExpr orderExprDecoration;
orderExprDecoration: (ASCENDING | DESCENDING | DESC)? (NULLS (FIRST | LAST))? (COLLATE STRING_LITERAL)?;
