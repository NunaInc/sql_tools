/**
   ...
   Original Author: Canwei He
      based on the Hive 3.1.2 grammar by Canwei He
   Changes: Copyright 2022 Nuna Inc
*/
...
// This is just a bad - to avoid columns named 'end' - should remove them from
// everywhere
functionArgument
    : (expression | nonReservedColumnName) (KW_AS identifier)?
    ;
...
functionalName
    : KW_EXISTS | KW_TRANSFORM | KW_TRANSFORM_KEYS | KW_TRANSFORM_VALUES
    | KW_FILTER | KW_ARRAY_SORT | KW_ZIP_WITH | KW_AGGREGATE | KW_FORALL
    | KW_MAP_FILTER | KW_MAP_ZIP_WITH
    ;
...
functional
    ...
    LPAREN
        ...
        (identifier | LPAREN aliasList RPAREN)
        PROCESSTO expression
    RPAREN
    ;
...
castExpression
    ...
    LPAREN
          expression
          KW_AS
          (primitiveType | listType | structType | mapType | unionType)
    RPAREN
    ;

caseExpression
    : KW_CASE expression
    (KW_WHEN expression KW_THEN (asExpression | multiNamedExpression))+
    (KW_ELSE (asExpression | multiNamedExpression))?
    KW_END?
    ;
...
multiNamedExpression
    : LPAREN
      asExpression (COMMA asExpression)*
      RPAREN
    ;
...
nonReserved
    ...
    | KW_OPTION | KW_OPTIONS | KW_OUTPUTDRIVER | KW_OUTPUTFORMAT | KW_OVERWRITE | KW_OWNER | KW_PARTITIONED | KW_PARTITIONS | KW_PLUS
    ...
    | KW_STREAMTABLE | KW_STRING | KW_STRUCT | KW_TABLES | KW_TBLPROPERTIES | KW_TEMPORARY | KW_TEMP | KW_TERMINATED
...
sql11ReservedKeywordsUsedAsFunctionName
    ...
    | KW_LEFT | KW_RIGHT | functionalName
    ;
