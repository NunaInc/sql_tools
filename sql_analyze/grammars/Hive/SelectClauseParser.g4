...
selectTrfmClause
    : LPAREN selectExpressionList RPAREN
      ...
      (KW_AS ((LPAREN (aliasList | columnNameTypeList) RPAREN)
...
selectItem
    : tableAllColumns
      | expression ((KW_AS? identifier) | (KW_AS LPAREN aliasList RPAREN))?
    ;

trfmClause
    : (KW_MAP selectExpressionList | KW_REDUCE selectExpressionList)
      rowFormat? recordWriter?
      KW_USING StringLiteral
      (KW_AS ((LPAREN (aliasList | columnNameTypeList) RPAREN)
              | (aliasList | columnNameTypeList)))?
...
window_frame_boundary
    ...
    | intervalExpression (KW_PRECEDING | KW_FOLLOWING )
    ;
